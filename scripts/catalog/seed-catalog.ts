...
function gujaratInstituteTypeToCollegeType(instituteType: string): string {
...
async function importGujaratFacts() {
  ...
  const { ensureCollege } = await buildCollegeResolver(prisma, {
    stateSlug: "gujarat",
    aliasSource: "gujarat_dump",
    canonicalSlugByMatchKey: GUJARAT_CANONICAL_SLUG_BY_MATCH_KEY,
    instituteTypeToCollegeType: gujaratInstituteTypeToCollegeType,
  });
...
async function main() {
  ...
  if (existsSync(GUJARAT_DUMP)) {
    await importGujaratFacts();
    await mergeGujaratDuplicateColleges();
  } else {
    console.warn(`Skipping Gujarat facts: missing ${GUJARAT_DUMP}`);
  }
